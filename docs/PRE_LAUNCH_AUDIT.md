...
- [x] `sale.controller.ts` — `createSale` (checkout/payment/stock-locking transaction) untested; `sales.test.ts` only covers `createPayment`, chart, stats, and list.
- [x] `purchase.controller.ts` — no test file at all (`createPurchase`, `deletePurchase`, overview stats, history).
- [x] `category.controller.ts` — no test file at all (create/update/delete, including parent/children cascade).
- [x] `admin.controller.ts` — no test file at all (invite, role update, deactivate, cancel-invite — all access-control-sensitive).
- [x] `product.controller.ts` — has a test file, but the `deleteById` success path (the cascading delete) is untested — only the 422 case is covered.

---

## Deployment checklist (Vercel + Railway split)

Not bugs, but must be set correctly before the split-origin deploy works. These need a dashboard action from you — not code, so left unchecked:

- [ ] **Vercel env vars:** `VITE_API_URL` (Railway server URL + `/api`), `VITE_CLERK_PUBLISHABLE_KEY`.
- [ ] **Railway env vars:** `DATABASE_URL`, `CLERK_SECRET_KEY`, `RESEND_API_KEY`, `CLIENT_URL`, `EMAIL_DOMAIN`, `ALLOWED_ORIGINS` (must exactly match the Vercel domain — `hono/cors` needs an exact origin match, not a wildcard, since `credentials: true` is set in production). `CLIENT_URL`/`EMAIL_DOMAIN` are now required at startup — see `server/.env.example` for the full list.
- [x] **`vercel.json` headers** — add the security headers/cache-control that Caddy used to provide (see Medium section above):
  ```json
  "headers": [
    { "source": "/(.*)", "headers": [
